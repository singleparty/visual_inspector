    ...
    <div class="vi_controllers" v-if="showPanel">
      ...
      <div class="vi_formLine vi_quickMatch">
        <Dropdown trigger="click">
          ...
          <DropdownMenu slot="list">
            ...
            <DropdownItem :key="index" v-for="(item, index) in wTypes" @click.native="wType = index">{{
              item
            }}</DropdownItem>
          </DropdownMenu>
        </Dropdown>
      ...
      <div class="vi_customSize vi_formLine" @keydown.stop @keyup.stop>
        <input
          ...
          type="text"
          class="vi_input"
          v-model.lazy="mockup.width"
          @keypress="handleCustomSizeInput"
        />
        <input
          title="height"
          placeholder="height"
      ...
      switch (val) {
        ...
        case 4: // 窗口居中
          this.moveAndResize({
            ...
            top: Math.max(
              0,
              (window.innerHeight - mockup.height) / 2 +
  ...
  methods: {
    ...
    reset() {
      ...
      this.moveAndResize({
        ...
        top: Math.max(
          0,
          (window.innerHeight - height) / 2 + Math.max(document.documentElement.scrollTop, document.body.scrollTop)
        ),
      })
